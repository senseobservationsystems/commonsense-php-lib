...
class Diagnostics {
	...
	public static function handleException(Exception $exception) {
		...
		if(@$_SERVER['headers_sent'] != 1) {
			...
			header("Access-Control-Allow-Headers: X-SESSION_ID, Content-Type");	
		}
		...
		if(RecessConf::$mode == RecessConf::DEVELOPMENT) {
			...
			if($exception instanceof RecessResponseException) {
				echo "{\"error\":\"".ResponseCodes::getMessageForCode($exception->responseCode)."\"}";
			} else {
				echo "{\"error\":\"".ResponseCodes::getMessageForCode(ResponseCodes::HTTP_INTERNAL_SERVER_ERROR)."\"}";
			}
